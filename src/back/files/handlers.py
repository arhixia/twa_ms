...
async def validate_and_process_attachment(attachment_id: int):
    async with SessionLocal() as db:
        async with db.begin():
            ...
            if not att:
                print(f"[DEBUG] validate_and_process_attachment: attachment {attachment_id} not found") 
                return
            print(f"[DEBUG] validate_and_process_attachment: processing {att.storage_key}, current processed={att.processed}") 
            s3 = get_s3_client()
            ...
            try:
                ...
                print(f"[DEBUG] S3 head failed: {e}") 
            ...
            if ctype not in ("image/jpeg", "image/png", "image/webp", "image/jpg"):
                print(f"[DEBUG] Invalid content type: {ctype}")
            ...
            try:
                ...
                print(f"[DEBUG] Thumbnail generated: {thumb_key}") 
            except Exception as e:
                print(f"[DEBUG] Thumb generation failed: {e}") 
                att.error_text = f"Thumb generation failed: {e}"

            att.processed = True
            att.error_text = att.error_text or None
            await db.flush()
            print(f"[DEBUG] Attachment {att.id} marked as processed=True") 
