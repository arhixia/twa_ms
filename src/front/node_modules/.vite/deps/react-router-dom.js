...
function createRouter(init) {
	...
	async function handleAction(request, location$1, submission, matches, scopedContext, isFogOfWar, initialHydration, opts = {}) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) result = {
			...
			let dsMatches = getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location$1, matches, actionMatch, initialHydration ? [] : hydrationRouteProperties2, scopedContext);
			let results = await callDataStrategy(request, location$1, dsMatches, scopedContext, null);
	...
	async function handleFetcherLoader(key, routeId, path, matches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		let result = (await callDataStrategy(fetchRequest, path, dsMatches, scopedContext, key))[match.route.id];
		if (fetchControllers.get(key) === abortController) fetchControllers.delete(key);
...
function createStaticHandler(routes, opts) {
	...
	async function submit(request, location$1, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest, filterMatchesToLoad, skipRevalidation) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) {
			...
			let dsMatches = getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, location$1, matches, actionMatch, [], requestContext);
			result = (await callDataStrategy(request, location$1, dsMatches, isRouteRequest, requestContext, dataStrategy))[actionMatch.route.id];
		...
		if (isErrorResult(result)) {
			let boundaryMatch = skipLoaderErrorBubbling ? actionMatch : findNearestBoundary(matches, actionMatch.route.id);
			return {
				...await loadRouteData(loaderRequest, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, filterMatchesToLoad, [boundaryMatch.route.id, result]),
				statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
				actionData: null,
				actionHeaders: { ...result.headers ? { [actionMatch.route.id]: result.headers } : {} }
			};
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, history, state, matches, submission, location$1, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, hasPatchRoutesOnNavigation, pendingActionResult, callSiteDefaultShouldRevalidate) {
	...
	let dsMatches = matches.map((match, index) => {
		...
		return getDataStrategyMatch(mapRouteProperties2, manifest, request, location$1, pattern, match, lazyRoutePropertiesToSkip, scopedContext, shouldLoad, shouldRevalidateArgs, callSiteDefaultShouldRevalidate);
	});
...
async function runMiddlewarePipeline(args, handler, processResult, isResult, errorHandler) {
	...
	return await callRouteMiddleware(dataFnArgs, tuples, handler, processResult, isResult, errorHandler);
}
