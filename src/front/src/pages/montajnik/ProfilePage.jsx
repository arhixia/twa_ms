...
const FILTERS_STORAGE_KEY = 'montajnikProfileFilters';
...
export default function ProfilePage() {
  ...
  const [showFilters, setShowFilters] = useState(() => {
    try {
      const saved = localStorage.getItem('montajnikProfileShowFilters');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('montajnikProfileShowFilters', JSON.stringify(showFilters));
    } catch (err) {
  ...
  const viewCompletedTask = (taskId) => {
    navigate(`/montajnik/completed-tasks/${taskId}`);
  };
