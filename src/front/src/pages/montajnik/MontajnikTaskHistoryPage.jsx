...
import {
  fetchMontTaskFullHistory, // Используем новую API функцию для монтажника
...
export default function MontajnikTaskHistoryPage() {
  ...
  // ✅ Обновляем loadRefs, чтобы загружать компании монтажника
  async function loadRefs() {
    try {
      const companiesData = await getMontCompaniesList(); // ✅ Используем эндпоинт монтажника
  ...
  async function loadHistory() {
    setLoading(true);
    try {
      const data = await fetchMontTaskFullHistory(id); // Вызываем API функцию для монтажника
      setHistory(data || []);
    } catch (err) {
  ...
  const getContactPersonNameById = useCallback(
    async (contactPersonId, companyId) => {
      ...
      if (!personsForCompany) {
        try {
          // Загружаем их с помощью эндпоинта монтажника
          personsForCompany = await getMontContactPersonsByCompany(companyId); // ✅ Используем эндпоинт монтажника
          // И обновляем кэш
...
function ContactNameResolver({ contactPersonId, companyId, getContactPersonNameById, contactPersonPhone }) {
  ...
  // ✅ Состояние для хранения загруженного/полученного телефона
  const [resolvedPhone, setResolvedPhone] = useState(contactPersonPhone || null); // <--- Добавлено
  ...
  useEffect(() => {
    ...
    async function resolveName() {
      if (!contactPersonId || !companyId) {
        ...
        if (!isCancelled) {
            setResolvedPhone(null); // <--- Добавлено
        }
        return;
      }
      ...
      try {
        ...
        // ✅ Если телефон не пришел из props (например, старая запись истории), попробуем загрузить его
        if (!contactPersonPhone) {
            try {
                const { phone } = await getMontContactPersonPhone(contactPersonId); // <--- Вызываем отдельный эндпоинт
                if (!isCancelled) {
