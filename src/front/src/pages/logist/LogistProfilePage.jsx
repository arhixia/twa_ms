...
const FILTERS_STORAGE_KEY = 'logistProfileFilters';
...
export default function LogistProfilePage() {
  ...
  const [showFilters, setShowFilters] = useState(() => {
    try {
      const saved = localStorage.getItem('logistProfileShowFilters');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('logistProfileShowFilters', JSON.stringify(showFilters));
    } catch (err) {
...
async function loadPeriodEarnings() {
  ...
  return (
    <div className="logist-main">
      <div className="page">
        <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isMobile ? '12px' : '16px' }}>
