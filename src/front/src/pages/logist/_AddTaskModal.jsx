  ...
  useEffect(() => {
    if (!searchTerm.trim()) {
      ...
      setFilteredCompanies(
        availableCompanies.filter(c => c.name.toLowerCase().includes(termLower))
      );
    }
...
function SearchableContactPersonSelect({ availableContactPersons, onSelect, selectedContactPersonId }) {
  ...
  useEffect(() => {
    if (!searchTerm.trim()) {
      ...
      const termLower = searchTerm.toLowerCase();
      setFilteredPersons(
        availableContactPersons.filter(cp => cp.name.toLowerCase().includes(termLower))
      );
    }
...
function SearchableEquipmentSelect({ availableEquipment, onSelect, selectedItems, equipmentList }) { // Передаем equipmentList
  const [searchTerm, setSearchTerm] = useState("");
  ...
  useEffect(() => {
    if (!searchTerm.trim()) {
      ...
      setFilteredEquipment(
        availableEquipment.filter(eq => eq.name.toLowerCase().includes(termLower))
      );
    }
  ...
  const handleItemClick = (equipmentId) => { // Принимаем ID
    onSelect(equipmentId); // Передаем ID
    setSearchTerm("");
  };
      ...
      {isOpen && filteredEquipment.length > 0 && (
        <ul className="searchable-select-dropdown">
          {filteredEquipment.map((eq) => (
            <li
              key={eq.id}
              onClick={() => handleItemClick(eq.id)} // Передаем ID
              className="searchable-select-option"
              onMouseDown={(e) => e.preventDefault()}
            >
              {eq.name}
...
function SearchableWorkTypeSelect({ availableWorkTypes, onSelect, selectedWorkTypeIds }) {
  ...
  useEffect(() => {
    if (!searchTerm.trim()) {
      ...
      setFilteredWorkTypes(
        availableWorkTypes.filter(wt => wt.name.toLowerCase().includes(termLower))
      );
    }
  ...
  const handleItemClick = (workTypeId) => { // Принимаем ID
    onSelect(workTypeId); // Передаем ID
    setSearchTerm("");
  };
      ...
      {isOpen && filteredWorkTypes.length > 0 && (
        <ul className="searchable-select-dropdown">
          {filteredWorkTypes.map((wt) => (
            <li
              key={wt.id}
              onClick={() => handleItemClick(wt.id)} // Передаем ID
              className="searchable-select-option"
              onMouseDown={(e) => e.preventDefault()}
            >
              {wt.name}
...
export default function AddTaskModal({ open, onClose, onSaved, allowSaveOnlyDraft = false }) {
  const [form, setForm] = useState({
    ...
    equipment: [],
    work_types_ids: [],
    district_id: null, // Добавляем новое поле
  });
  ...
  const [districts, setDistricts] = useState([]); // Добавляем состояние для районов
  ...
  async function loadRefs() {
    try {
      ...
      // Загружаем районы для текущего пользователя (его компании)
      try {
        const distData = await getSimpleDistricts(); // Используем новый метод
  ...
  // ... (handleCompanyChange остается как было, но не используется для районов)
  ...
  function addEquipmentItem(equipmentId) { // Принимает ID
    if (!equipmentId) return;
    const eq = equipmentList.find(e => e.id === equipmentId); // Использует equipmentList из области видимости
    if (!eq) return;
    const newItem = {
    ...
    setForm((prevForm) => ({ // Использует setForm из области видимости
      ...prevForm,
  ...
  function addWorkType(workTypeId) { // Принимает ID
                ...
                <div className="task-field-value">
                  <div className="equipment-list-container" style={{ marginBottom: '8px' }}>
                    {form.equipment.map((item, index) => {
                      const eq = equipmentList.find((e) => e.id === item.equipment_id); // equipmentList из области видимости
                      return (
                        <div key={index} className="equipment-item-row">
                          ...
                          <input
                            ...
                            onChange={(e) => updateEquipmentItem(index, "serial_number", e.target.value)} // updateEquipmentItem из области видимости
                            className="equipment-item-serial"
                          />
                          <button
                            type="button"
                            onClick={() => removeEquipmentItem(index)} // removeEquipmentItem из области видимости
                  ...
                  <SearchableEquipmentSelect
                    availableEquipment={equipmentList} // Передаем список
                    onSelect={addEquipmentItem} // Передаем функцию
                    selectedItems={form.equipment}
                    equipmentList={equipmentList} // Передаем список как пропс, если нужно внутри
                  />
                </div>
              </div>
              <div className="task-field">
                ...
                <div className="task-field-value">
                  <div className="work-types-container" style={{ marginBottom: '8px' }}>
                    {(() => {
                      ...
                      return uniqueWorkTypesWithCounts.map(({ id, count }) => {
                        const wt = workTypesList.find((w) => w.id === id); // workTypesList из области видимости
                        if (!wt) return null;
                        return (
                          <div key={id} className="work-type-tag" style={{ display: 'inline-block', margin: '0 4px 4px 0' }}>
                            ...
                            <span
                              ...
                              onClick={() => removeWorkType(id)} // removeWorkType из области видимости
                  ...
                  <SearchableWorkTypeSelect
                    availableWorkTypes={workTypesList} // Передаем список
                    onSelect={addWorkType} // Передаем функцию
      ...
      {assignmentTypeOptions.map(option => (
        <option key={option.value} value={option.value}>
          {option.display}
              ...
              {/* Условный рендеринг поля выбора района */}
                ...
                <div className="task-field">
                  ...
                  <div className="task-field-value">
                    <SearchableDistrictSelect
                      ...
                      onSelect={(districtId) => setField("district_id", districtId)} // setField из области видимости
                  ...
                  <div className="task-field-value">
                    {form.assigned_user_id && (
                      <div style={{ padding: '6px 10px', marginBottom: '8px', border: '1px solid #30363d', borderRadius: '4px', backgroundColor: '#161b22', color: '#c9d1d9', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <button
                          type="button"
                          onClick={() => setField("assigned_user_id", null)} // setField из области видимости
                    ...
                    <SearchableMontajnikSelect
                      ...
                      onSelect={(userId) => setField("assigned_user_id", userId)} // setField из области видимости
              ...
              <div className="task-field">
                ...
                <div className="task-field-value">
                  <textarea
                    value={form.comment || ""}
                    onChange={(e) => setField("comment", e.target.value)} // setField из области видимости
                    rows="3"
                    className="dark-select"
                    style={{ width: '100%', resize: "vertical" }}
                  />
        ...
        <div className="modal-actions">
          <button
            onClick={() => saveDraft(false)} // saveDraft из области видимости
            disabled={saving}
          ...
          {!allowSaveOnlyDraft && (
            <button
              onClick={() => saveDraft(true)} // saveDraft из области видимости
              disabled={saving}
              className="gradient-button"
            >
              {saving ? 'Публикация...' : (
