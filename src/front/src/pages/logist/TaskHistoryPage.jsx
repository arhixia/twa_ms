// front/src/pages/logist/TaskHistoryPage.jsx
...
export default function TaskHistoryPage() {
  ...
  async function loadHistory() {
    setLoading(true);
    try {
      const data = await fetchTaskFullHistory(id);
      setHistory(data || []);
    } catch (err) {
  ...
  const getContactPersonNameById = useCallback(
    async (contactPersonId, companyId) => {
      ...
      if (!personsForCompany) {
        try {
          personsForCompany = await getContactPersonsByCompany(companyId);
    ...
    async function resolveName() {
      ...
      try {
        ...
        if (!contactPersonPhone) {
            try {
                const { phone } = await getContactPersonPhone(contactPersonId);
                if (!isCancelled) {
