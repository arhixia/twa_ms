...
export default function AdminTaskHistoryPage() {
  ...
  async function loadRefs() {
    try {
      const companiesData = await getAdminCompaniesList();
  ...
  async function loadHistory() {
    setLoading(true);
    try {
      const data = await adminfetchTaskFullHistory(id);
      setHistory(data || []);
    } catch (err) {
  ...
  const getContactPersonNameById = useCallback(
    async (contactPersonId, companyId) => {
      ...
      if (!personsForCompany) {
        try {
          personsForCompany = await getAdminContactPersonsByCompany(companyId);
            ...
            return (
              <li key={h.id} style={{ 
                ...
                <div style={{ 
                  ...
                  <b style={{ marginBottom: '8px', display: 'block' }}>Состояние задачи:</b>
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      ...
                      <ContactNameResolver
                        ...
                        contactPersonPhone={h.contact_person_phone}
                      />
                    </div>
...
function ContactNameResolver({ contactPersonId, companyId, getContactPersonNameById, contactPersonPhone }) {
  ...
  const [resolvedPhone, setResolvedPhone] = useState(contactPersonPhone || null);
    ...
    async function resolveName() {
      ...
      try {
        ...
        if (!contactPersonPhone) {
            try {
                const { phone } = await getAdminContactPersonPhone(contactPersonId);
                if (!isCancelled) {
  ...
  }, [contactPersonId, companyId, getContactPersonNameById, contactPersonPhone]);
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      {resolvedPhone && (
        <span>
          (<a href={`tel:${resolvedPhone}`} style={{ color: '#1e88e5', textDecoration: 'none' }}>{resolvedPhone}</a>)
        </span>
      )}
    </div>
  );
